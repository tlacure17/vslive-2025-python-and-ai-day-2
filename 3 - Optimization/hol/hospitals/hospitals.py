...
# Clean up old images
for old_file in glob.glob("hospitals*.png") + glob.glob("hospitals*.gif"):
    os.remove(old_file)

# Use local search to determine hospital placement
hospitals = s.hill_climb(image_prefix="hospitals", log=True)

# Create a GIF of the movements
images = []

for filename in sorted(glob.glob("hospitals*.png")):
    image = Image.open(filename)
    images.append(image)

if images:
    # Add extra copies of the last frame to create a pause before looping
    for _ in range(10):  # Adds 3 seconds pause (10 frames * 300ms)
        images.append(images[-1].copy())
    images[0].save("hospitals.gif", save_all=True, append_images=images[1:], duration=300, loop=0)